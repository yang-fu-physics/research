...
Two-band Hall conductivity fitting program.
- Input : measured rho_xx (ohm cm) and rho_xy (ohm cm) vs B
- Converts inputs to conductivity: sigma_xx, sigma_xy
- Fits sigma_xy(B) (Hall conductivity) with sigma_xx(0) as a soft constraint
  (analogous to twobandfit.py which fits rho_xy with rho_xx(0) as constraint)
- Output : fitted carrier concentrations & mobilities, sigma_xy fit plot
...
def resample_uniform(*arrays_B_and_data):
    ...
    Parameters
    ----------
    *arrays_B_and_data : alternating (B_array, data_array) pairs

    Returns
    -------
    B_uniform       : uniform B grid
    *data_resampled : one resampled data array per input pair
    """
    # Unpack pairs
...
# ===== Core Fitting: sigma_xy only, sigma_xx(0) constraint =====

def fit_two_band_sigma(B_xy, sigma_xy_data, sigma_xx_0=None):
    """
    Fit sigma_xy(B) with an optional sigma_xx(0) soft constraint.

    Parameters
    ----------
    B_xy          : B field array (T) for Hall conductivity
    sigma_xy_data : sigma_xy values (S/m)
    sigma_xx_0    : target zero-field longitudinal conductivity (S/m), optional

    Returns
    -------
    popt  : [n1s, mu1, n2s, mu2]  (n in 1e20 m^-3)
    perr  : 1-sigma errors
    r2_xy : R^2 of the sigma_xy fit
    """
    # ---- Initial guess from low-field Hall slope ----
    mask = (np.abs(B_xy) > 0.3) & (np.abs(B_xy) < 1.5)
    if mask.sum() > 4:
        slope = np.polyfit(B_xy[mask], sigma_xy_data[mask], 1)[0]
    else:
        slope = (sigma_xy_data[-1] - sigma_xy_data[0]) / (B_xy[-1] - B_xy[0] + 1e-30)

    n_sign = -1.0 if slope < 0 else 1.0
    ref_sigma = sigma_xx_0 if sigma_xx_0 is not None else np.max(np.abs(sigma_xy_data)) * 10
    mu_guess = abs(slope) / (ref_sigma + 1e-30)
    mu_guess = np.clip(mu_guess, 1e-4, 100.0)
    n_guess  = ref_sigma / (e_charge * mu_guess)
    ...
    def residual(params):
        try:
            ...
            pred = model_sigma_xy(B_xy, *params)
            err  = np.sum((pred - sigma_xy_data)**2)
            # sigma_xx(0) soft constraint (analogous to twobandfit.py rho_xx(0) constraint)
            if sigma_xx_0 is not None:
                s_pred = e_charge * abs(n1s * 1e20) * mu1 + e_charge * abs(n2s * 1e20) * mu2
                w = 100 * np.sum((sigma_xy_data - sigma_xy_data.mean())**2) / sigma_xx_0**2
    ...
    de_res = differential_evolution(
        residual, bounds_de, seed=42, maxiter=2000, tol=1e-14,
    ...
    res_var = residual(popt) / (len(B_xy) - n_params)
    try:
    ...
    r2_xy = 1 - np.sum((model_sigma_xy(B_xy, *popt) - sigma_xy_data)**2) / \
...
def main():
    ...
    print("=" * 58)
    print("   Two-Band Hall Conductivity Fit (sigma_xy + constraint)")
    print("=" * 58)

    # --- Load rho_xx data (for sigma_xx(0) constraint) ---
    ...
    # Extract sigma_xx(0) from rho_xx at B~0
    idx_zero = np.argmin(np.abs(B_xx))
    rho_xx_0_cm  = rho_xx_cm[idx_zero]
    sigma_xx_0   = 1.0 / (rho_xx_0_cm * 1e-2)   # S/m
    print(f"-> Auto-extracted constraint: rho_xx(0) = {rho_xx_0_cm:.4e} ohm cm "
          f"→ sigma_xx(0) = {sigma_xx_0:.4e} S/m  (at B = {B_xx[idx_zero]:.4f} T)")
    ...
    # Resample both datasets onto a common uniform B grid, then convert to sigma_xy
    B_fit, rho_xx_fit, rho_xy_fit = resample_uniform(
        B_xx, rho_xx_m, B_xy, rho_xy_m
    )
    _, sigma_xy_data = rho_to_sigma(rho_xx_fit, rho_xy_fit)
    print(f"-> Uniform B grid: {len(B_fit)} points  "
    ...
    popt, perr, r2_xy = fit_two_band_sigma(B_fit, sigma_xy_data, sigma_xx_0)
    ...
    print(f"sigma_xx(0) from data : {sigma_xx_0:.6e} S/m  [= rho_xx(0) = {rho_xx_0_cm:.6e} ohm cm]")
    print(f"sigma_xx(0) from fit  : {sigma_xx_0_fit:.6e} S/m  [= rho_xx(0) = {rho_xx_0_fit_cm:.6e} ohm cm]")
    print(f"R^2 (sigma_xy fit)    : {r2_xy:.4f}\n")

    # Compute sigma_xx from the rho_xx data directly (for comparison, not fitted)
    rho_xy_at_xx = np.interp(B_xx, np.sort(B_xy), rho_xy_m[np.argsort(B_xy)])
    sigma_xx_data_full, _ = rho_to_sigma(rho_xx_m, rho_xy_at_xx)
    sigma_xx_calc = model_sigma_xx(B_xx, *popt)
    r2_xx_compare = 1 - np.sum((sigma_xx_calc - sigma_xx_data_full)**2) / \
                        np.sum((sigma_xx_data_full - sigma_xx_data_full.mean())**2)
    print(f"R^2 (sigma_xx compare, not fitted): {r2_xx_compare:.4f}")

    # ===== Visualization =====
    B_smooth = np.linspace(min(B_fit.min(), B_xx.min()), max(B_fit.max(), B_xx.max()), 600)
    ...
    fig.suptitle("Two-Band Hall Conductivity Fit", fontsize=15, fontweight='bold')

    # Left: sigma_xx (comparison only, not fitted)
    ax_xx = axes[0]
    ax_xx.scatter(B_xx, sigma_xx_data_full, color='black', marker='o', facecolors='none',
    ...
    ax_xx.annotate(f'Compare $R^2$ = {r2_xx_compare:.4f}', xy=(0.05, 0.94),
                   xycoords='axes fraction', fontsize=11, color='blue', va='top')
    ax_xx.grid(True, ls='--', alpha=0.5)

    # Right: sigma_xy (fitted)
    ax_xy = axes[1]
    ax_xy.scatter(B_fit, sigma_xy_data, color='black', marker='o', facecolors='none',
                  s=12, label='From data ($\\sigma_{xy}$)')
    ax_xy.plot(B_smooth, sxy_fit_smooth, 'r-', lw=2, label='Two-band Fit')
    ...
    out_png = base_name + '_sigma_fit_plot.png'
    out_dat = base_name + '_sigma_fit_result.dat'
    ...
    try:
        ...
        sxx_at_xx   = model_sigma_xx(B_xx, *popt)
        with open(out_dat, 'w') as f:
            f.write("# Two-Band Hall Conductivity Fit Results\n")
            f.write(f"# Fit target: sigma_xy(B), constraint: sigma_xx(0)\n")
            f.write(f"# R2(sigma_xy fit)={r2_xy:.6f}  R2(sigma_xx compare)={r2_xx_compare:.6f}\n")
            f.write(f"# sigma_xx(0) from data = {sigma_xx_0:.6e} S/m\n")
            f.write(f"# sigma_xx(0) from fit  = {sigma_xx_0_fit:.6e} S/m\n")
            f.write(f"# rho_xx(0) from data   = {rho_xx_0_cm:.6e} ohm cm\n")
            ...
            f.write("# ------------------\n")
            f.write("# [Hall conductivity fit @ Hall B grid]\n")
            f.write("# B(T)\tsigma_xy_data(S/m)\tsigma_xy_fit(S/m)\n")
            for i in range(len(B_fit)):
                f.write(f"{B_fit[i]:.6f}\t{sigma_xy_data[i]:.6e}\t{sxy_at_data[i]:.6e}\n")
            f.write("# [sigma_xx comparison @ rho_xx B grid]\n")
            f.write("# B(T)\tsigma_xx_data(S/m)\tsigma_xx_calc(S/m)\n")
            for i in range(len(B_xx)):
                f.write(f"{B_xx[i]:.6f}\t{sigma_xx_data_full[i]:.6e}\t{sxx_at_xx[i]:.6e}\n")
