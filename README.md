...
主要是在科研中使用的一些代码，用于霍尔效应和电阻数据处理。

## 1. datadeal.py（组内最常用的功能）
...
现在支持通过 Streamlit Web 界面使用，更加直观便捷！
...
# 启动 Web 界面
streamlit run datadeal_app.py
```

浏览器会自动打开 `http://localhost:8501`，界面功能包括：
- 📤 **上传数据文件**：直接拖拽或选择 .dat 文件上传
- ⚙️ **可视化配置**：内插分段、样品尺寸等参数
- 📊 **实时预览**：处理过程中显示图像
- 🔬 **一键拟合**：双带拟合和 RH 拟合
- 📦 **打包下载**：将 data/、fit/ 文件夹和 alldata.png 打包为 ZIP 下载
- 🗑️ **清理功能**：一键清除数据文件或结果文件夹
...
主要是一些小工具：
- `2harm.py` 和 `plotnolineardata.py`：处理非线性霍尔的数据，并作图
- `search_cutpage_pdf.py`：写了一个 pdf 搜索，摘取某些页数的程序，主要是为了摘取其中的有基金号部分。
